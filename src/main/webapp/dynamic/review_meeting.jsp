		...
		<table>
			<tr>
				...
				if (action != null && action.equals("insert")) {
					...
					("INSERT INTO review_meeting VALUES (?,?,?)"));
					
					pstmt.setInt(1, Integer.parseInt(request.getParameter("REVIEW_MEETING_ID")));
					pstmt.setTimestamp(2, Timestamp.valueOf(request.getParameter("START_TIME")));
					pstmt.setTimestamp(3, Timestamp.valueOf(request.getParameter("END_TIME")));
				...
				if (action != null && action.equals("update")) {
					...
					PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE review_meeting SET END_TIME = ? WHERE REVIEW_MEETING_ID = ? AND START_TIME = ?");
					
					pstmt.setTimestamp(1, Timestamp.valueOf(request.getParameter("END_TIME")));
					pstmt.setInt(2, Integer.parseInt(request.getParameter("REVIEW_MEETING_ID")));
				...
				if (action != null && action.equals("delete")) {
					...
					PreparedStatement pstmt = conn.prepareStatement(
					"DELETE FROM review_meeting WHERE REVIEW_MEETING_ID = ? AND START_TIME = ?");
				...
				ResultSet rs = statement.executeQuery("SELECT * FROM review_meeting");
				%>
				...
				%>
					<tr>
						...
						<form action="review_meeting.jsp" method="get">
							...
							<input type="hidden" value="<%= rs.getInt("REVIEW_MEETING_ID") %>" name="REVIEW_MEETING_ID">
